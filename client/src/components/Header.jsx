      ...
      <div className='flex justify-between items-center max-w-6xl mx-auto p-3'>
        <Link to='/'>
          <h1 className='font-bold text-sm sm:text-xl flex flex-wrap'>
            <span className='text-zinc-900'>Lambrecht</span>
        ...
        <ul className='flex gap-4'>
          ...
          <Link to='/profile'>{currentUser ? <img className='rounded-full h-7 w-7 object-cover' src={currentUser.avatar} alt='profile' /> : <li className='text-stone-200 hover:underline'>Sign In</li>}</Link>
        </ul>
      </div>
    </header>
  );
