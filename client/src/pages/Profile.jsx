...
export default function Profile() {
  ...
  const handledListingDelete = async (listingId) => {
    try {
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <img className='rounded-full h-24 w-24 object-cover cursor-pointer self-center mt-2' onClick={() => fileRef.current.click()} src={formData.avatar || currentUser.avatar} alt='profile' />
        ...
        <input className='border p-3 rounded-lg' type='text' placeholder='email' defaultValue={currentUser.email} id='email' onChange={handleChange} />
        ...
        <div className='flex flex-col gap-4'>
          ...
          {userListings.map((listing) => (
            <div key={listing._id} className='border rounded-lg p-3 flex justify-between items-center gap-4'>
              ...
              <Link className='font-semibold flex-1 hover:underline truncate' to={`/listing/${listing._id}`}>
                <p className='text-slate-700'>{listing.name}</p>
              </Link>
              <div className='flex flex-col items-center'>
                <button onClick={() => handledListingDelete(listing._id)} className='text-red-700 uppercase'>
                  Delete
                </button>
